    ...
    <>
      ...
      <div className="fixed top-4 left-0 right-0 z-50 px-4 md:px-6">
        <motion.nav
          ...
          className={cn(
            'max-w-5xl mx-auto flex h-12 items-center justify-between px-4 rounded-2xl transition-all duration-300',
            scrolled
              ? 'border border-[hsl(var(--border))] bg-[hsl(var(--bg)/0.85)] backdrop-blur-xl shadow-xl shadow-black/10'
              : 'border border-[hsl(var(--border)/0.5)] bg-[hsl(var(--bg)/0.6)] backdrop-blur-md'
          )}
        >
          {/* Logo */}
          <Link href="/" className="flex items-center gap-2 font-semibold text-sm shrink-0">
            <div className="w-6 h-6 rounded-lg bg-[hsl(var(--brand))] flex items-center justify-center shadow-[0_2px_8px_hsl(var(--brand)/0.4)]">
              <Sparkles className="w-3 h-3 text-white" />
            </div>
            <span className="tracking-[-0.03em]">Brainigen</span>
          </Link>
            ...
            {navLinks.map(({ href, key }) => {
              ...
              return (
                <Link
                  ...
                  href={href}
                  className={cn(
                    'px-2.5 py-1.5 text-sm rounded-lg transition-colors duration-150',
                    isActive
                      ? 'text-[hsl(var(--fg))] font-medium bg-[hsl(var(--surface-2))]'
                      : 'text-[hsl(var(--muted))] hover:text-[hsl(var(--fg))] hover:bg-[hsl(var(--surface-2)/0.7)]'
                  )}
                >
                  {t(key)}
                </Link>
          ...
          <div className="hidden md:flex items-center gap-2">
            {loading ? (
              <div className="h-7 w-7 rounded-full bg-[hsl(var(--surface-2))] animate-pulse" />
            ) : user ? (
              <DropdownMenu>
                <DropdownMenuTrigger className="flex items-center gap-2 rounded-full focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[hsl(var(--brand))] cursor-pointer">
                  <Avatar className="h-7 w-7 ring-2 ring-[hsl(var(--border))]">
                    <AvatarImage src={profile?.avatar_url || undefined} />
                    <AvatarFallback className="text-[10px] bg-[hsl(var(--brand)/0.1)] text-[hsl(var(--brand))]">{initials}</AvatarFallback>
                  </Avatar>
                </DropdownMenuTrigger>
              ...
              <>
                <Link
                  href="/login"
                  className="px-3 py-1.5 text-sm text-[hsl(var(--muted))] hover:text-[hsl(var(--fg))] transition-colors"
                >
                  {t('login')}
                </Link>
                <Link
                  href="/register"
                  className="inline-flex items-center h-8 px-4 bg-[hsl(var(--brand))] text-white rounded-lg text-sm font-medium hover:bg-[hsl(var(--brand)/0.9)] transition-all shadow-[0_2px_8px_hsl(var(--brand)/0.3)] hover:shadow-[0_4px_12px_hsl(var(--brand)/0.4)] active:scale-[0.97] cursor-pointer"
                >
                  {t('signup')}
                </Link>
              </>
          ...
          <button
            ...
            className="md:hidden flex items-center justify-center h-8 w-8 rounded-lg hover:bg-[hsl(var(--surface-2))] transition-colors cursor-pointer"
      ...
      <AnimatePresence>
        {mobileOpen && (
          <motion.div
            ...
            className="fixed inset-0 top-20 z-40 bg-[hsl(var(--bg))/0.98] backdrop-blur-xl md:hidden flex flex-col border-t border-[hsl(var(--border))]"
          >
              ...
              <div className="border-t border-[hsl(var(--border))] pt-6 space-y-3">
                {user ? (
                  ...
                  <>
                    ...
                    <Link href="/register" className="flex items-center justify-center h-12 px-4 rounded-xl bg-[hsl(var(--brand))] text-white text-sm font-semibold hover:bg-[hsl(var(--brand)/0.9)] transition-colors cursor-pointer shadow-[0_4px_12px_hsl(var(--brand)/0.3)]">
                      {t('signup')}
                    </Link>
                  </>
                )}
